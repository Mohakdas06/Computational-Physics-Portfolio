...
! Topic   : FDM approximation for Half-quartic potential
! Units   : m = h_bar = 1
...
program prob11_driver
    ...
    open(unit=10, file='data/prob11_energies.dat', status='replace')
    do i = 1, 5
    ...
    open(unit=10, file='data/prob11.dat', status='replace')
    do i = 1, N
        x = real(i, dp) * dx
        write(10, *) x, potential(x), z(i,1), z(i, 2), z(i, 3), z(i, 4), z(i, 5)
    end do
    close(10)
    write(*, '(A)') "Wavefunctions written to prob11.dat"
end program prob11_driver