...
! Topic   : FDM approximation for Quartic (x^4) potential
! Units   : m = h_bar = 1
...
program prob08_driver
    ...
    open(unit=10, file='data/prob08_energies.dat', status='replace')
    do i = 1, 5
    ...
    open(unit=10, file='data/prob08.dat', status='replace')
    do i = 1, N
        x = - L_box + real(i-1, dp) * dx
        write(10, *) x, potential(x), z(i,1), z(i,2), z(i,3), z(i, 4), z(i, 5)
    end do
    close(10)
    print *, "Wavefunctions written to prob08.dat"
