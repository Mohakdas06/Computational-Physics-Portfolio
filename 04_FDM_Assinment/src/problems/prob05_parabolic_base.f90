...
! Topic   : FDM approximation for Infinite Potential Well With Parabolic Base
! Units   : m = h_bar = 1
...
module prob05_physics
    use constants_mod
    implicit none
    real(dp), parameter :: V0 = 20000.0_dp
...
program prob05_driver
    ...
    open(unit=10, file='data/prob05_energies.dat', status='replace')
    do i = 1, 5
    ...
    open(unit=10, file='data/prob05.dat', status='replace')
    do i = 1, N
        x = real(i, dp) * dx
        write(10, *) x, potential(x),z(i,1), z(i, 2), z(i, 3), z(i, 4), z(i, 5)
    end do
    close(10)
    write(*, '(A)') "Wavefunctions written to prob05.dat"
