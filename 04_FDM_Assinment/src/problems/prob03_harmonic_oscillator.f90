!===========================================================
! File    : prob03_harmonic_oscillator.f90
! Author  : Mohak Das
...
program prob03_driver
    ...
    write(*, '(A)') "----------------------------------------------"
    write(*, '(A)') "Problem 3: Harmonic Oscillator"
    write(*, '(A)') "----------------------------------------------"
    ...
    open(unit=10, file='data/prob03_energies.dat', status='replace')
    do i = 1, 5
    ...
    open(unit=10, file='data/prob03.dat', status='replace')
    do i = 1, N
        x = - L + (i - 1) * dx
        write(10, *) x, potential(x), z(i,1), z(i,2), z(i,3), z(i, 4), z(i, 5)
    end do
    close(10)
    print *, "Wavefunctions written to prob03.dat"
